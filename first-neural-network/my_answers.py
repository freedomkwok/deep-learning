...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        hidden_inputs = np.dot(np.array([X]), self.weights_input_to_hidden)
        ...
        return final_outputs, final_inputs
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        print("weights_hidden_to_output", self.weights_hidden_to_output.shape, output_error_term.shape)
        ...
        print("np.array([X])", np.array([X]).shape, "hidden_error_term", hidden_error_term.shape)
        # Weight step (input to hidden)
        delta_weights_i_h += np.dot(hidden_error_term, np.array([X])).T #3x2

        # Weight step (hidden to output)
        delta_weights_h_o += output_error_term * hidden_outputs #2x1
